...
export default function Contact() {
    ...
    return (
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            <div>Name</div>
            <input className="form__input" {...register("firstname")} onChange={e => setMessageName(e.target.value)} value={messageName} />
            {errors.firstname && <span className="error__message">{errors.firstname.message}</span>}

            <div>Email</div>
            <input className="form__input" {...register("email")} onChange={e => setMessageEmail(e.target.value)} value={messageEmail}/>
            {errors.email && <span className="error__message">{errors.email.message}</span>}

            <div>Your Message</div>
            <input className="form__input__message" {...register("message")} onChange={e => setMessageMessage(e.target.value)} value={messageMessage} />
            {errors.message && <span className="error__message">{errors.message.message}</span>}



            <button className="default__button" onClick={() => addMessage() } >Send</button>
